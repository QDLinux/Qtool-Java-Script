...
if(type==0) filename="RS01"+name+".flac";
	...
	try
	{
		...
		String text = http.getWithAgentCookie("http://u.y.qq.com/cgi-bin/musicu.fcg?data=%7B%22req_0%22%3A%7B%22module%22%3A%22vkey.GetVkeyServer%22%2C%22method%22%3A%22CgiGetVkey%22%2C%22param%22%3A%7B%22guid%22%3A%220%22%2C%22filename%22%3A%5B%22"+filename+"%22%5D%2C%22songmid%22%3A%5B%22"+SongMid+"%22%5D%2C%22songtype%22%3A%5B0%5D%2C%22uin%22%3A%22"+MyUin+"%22%2C%22loginflag%22%3A1%2C%22platform%22%3A%2220%22%7D%7D%7D","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.105 Safari/537.36",COOKIE);
...
public String GetQQMusicSongUrl(String SongMid)
{
	...
	//String cookie="p_uin=o0"+MyUin+";skey="+getSkey()+";p_skey="+getPskey("y.qq.com");
	if(COOKIE==null)login(null);
		String text = http.getWithAgentCookie("http://u6.y.qq.com/cgi-bin/musicu.fcg?data=%7B%22req_0%22%3A%7B%22module%22%3A%22vkey.GetVkeyServer%22%2C%22method%22%3A%22CgiGetVkey%22%2C%22param%22%3A%7B%22guid%22%3A%220%22%2C%22songmid%22%3A%5B%22"+SongMid+"%22%5D%2C%22songtype%22%3A%5B0%5D%2C%22uin%22%3A%22"+MyUin+"%22%2C%22loginflag%22%3A1%2C%22platform%22%3A%2220%22%7D%7D%7D","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.105 Safari/537.36",COOKIE);
		JSONObject json = new JSONObject(text);
            String purl=json.get("req_0").get("data").get("midurlinfo").get(0).get("purl");
		String SongUrl = "http://isure.stream.qqmusic.qq.com/"+purl;
		if(!SongUrl.contains("vkey")){
...
public static String SearchQQMusicList(String searchName)
{
    if(searchName != null && !searchName.isEmpty())
    {
        try
        {
            ...
            String jsonStr = http.get("http://u.y.qq.com/cgi-bin/musicu.fcg?data=%7B%22comm%22%3A%7B%22ct%22%3A%221%22%2C%22cv%22%3A%2210180005%22%2C%22vG%22%3A%2210180005%22%7D%2C%22req%22%3A%7B%22method%22%3A%22DoSearchForQQMusicMobile%22%2C%22module%22%3A%22music.search.SearchBrokerCgiServer%22%2C%22param%22%3A%7B%22search_type%22%3A0%2C%22query%22%3A%22" + searchName + "%22%2C%22page_num%22%3A1%2C%22num_per_page%22%3A"+"50"+"%2C%22grp%22%3A0%7D%7D%7D");
...
if(delFile==1) delAllFile(new File(path),0);
		...
		//String audio=json.getString("playurl");
		//String img=json.getString("pic");
		if(data.IsChannel)sendChannelMusic( qun, title, desc, detail_url, audio, img);
		else if(data.IsGroup)sendTroopMusic( qun, title, desc, detail_url, audio, img);
		else {
		if(qun==null||qun.equals("")) sendFriendMusic(qq, title, desc, detail_url, audio, img);
		else {
		sendTroopMemberMusic( qun,qq, title, desc, detail_url, audio, img);
		}
		/* json=new JSONObject((json+"").replace("&","&amp;"));
			String JsonXml = MusicXml;
			JsonXml = JsonXml.replace("{标题}", json.getString("name")).replace("{作者}", json.getString("singer")).replace("{图标}", json.getString("pic")).replace("{链接}", json.getString("playurl"));
			JsonXml = JsonXml.replace("{歌曲}", (json.getString("playurl")));
			JsonXml = JsonXml.replace("{群号}", qun);
