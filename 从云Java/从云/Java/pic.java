...
public pic(String text,String qun)
{
String texc="";
if(text.contains("[PicUrl=")||text.contains("http"))
{
sendMsg(qun,"",zz+"\n"+text);
return;
}
if(text.contains("[AtQQ="))
{
text=text.replace("[AtQQ="+uin2,"").replace("]","@"+CT);
}
texc=TextUtils.emoticonToTextWithoutSysEmotion(text);
String yy=AppPath+"/从云/图片缓存/"+sco()+".png";
String col=sco();
if(!colord2.equals("")){
col=colord2;
}
texttopng(texc,colord1,sco(),size1,yy);
