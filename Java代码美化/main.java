...
public void 主要部分(String qun){
    加载活动();
    活动.runOnUiThread(new Runnable(){
        public void run(){
            EditText code = new EditText(活动);
            new AlertDialog.Builder(活动,AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("输入待美化代码").setView(code).setNeutralButton("我写在文件里了！",new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialogInterface, int i) {
                    new fileSystem(RootPath+"out.java").write(beautiful(new fileSystem(RootPath+"in.java").read()));
                    Toast("美化完毕，请前往脚本目录下的out.java查看");
                }
            }).setNegativeButton("指定路径(会覆盖原文件)",new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialogInterface, int i) {
                    加载活动();
                    活动.runOnUiThread(new Runnable(){
                        public void run(){
                            EditText et2 = new EditText(活动);
                            new AlertDialog.Builder(活动,AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("请指定路径(绝对路径)").setView(et2).setPositiveButton("确定",new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    new fileSystem(""+et2.getText()).write(beautiful(new fileSystem(""+et2.getText()).read()));
                                    Toast("美化成功");
                                }
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    ...
                    活动.runOnUiThread(new Runnable(){
                        public void run(){
                            String t = beautiful(""+code.getText());
                            EditText et = new EditText(活动);
                            et.setText(t.toCharArray(),0,t.length());
                            new AlertDialog.Builder(活动,AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("美化结果").setView(et).setPositiveButton("确定",null).show();
                        }
                    });
                }
            }).show();
