...
String result=post("https://u.y.qq.com/cgi-bin/musicu.fcg",req);
...
try{
String tt=getWithAgentCookie("https://c.y.qq.com/portalcgi/fcgi-bin/music_mini_portal/fcg_vip_score_rank_new.fcg?format=json&outCharset=utf-8&cmd=1","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.105 Safari/537.36",COOKIE);
...
public void run(){
if(json==null||json.optBoolean("ReLogin",false))
{
 if(json == null) json=getloginjson();
if(json.optBoolean("ReLogin",false)|| !checklogin(json))
{
Toastm("COOKIE过期或不存在重新登录");
putString(LoginUin, "COOKIE", null);
COOKIE=accountlogin();
json=getloginjson();
if(json.optBoolean("limitlogin",false)){
Object uin= json.get("uin");
    String nick=json.get("nick");
    String qqHead=json.get("qqHead");
    download(qqHead,Mpath+"/图片/my.jpg");
    Toastm("QQ音乐\""+nick+"\"("+uin+")\n已使用Pskey替代Cookie!");
}
else if(checklogin(json))
{
   ...
   // String follow=json.get("followers_count_str");
    String qqHead=json.get("qqHead");
    download(qqHead,Mpath+"/图片/my.jpg");
    Toastm("QQ音乐\""+nick+"\"("+uin+")Key校验成功!\nCOOKIE已存储。");
}
else{
    Toastm(COOKIE+"\n"+json);
    Toastm("QQ音乐登录失败，可能未绑定QQ账号或者存在其它问题\n请去QQ音乐客户端用QQ授权登录QQ音乐\n然后发送\"登录QQ音乐\"重新登录");
    putString(LoginUin, "COOKIE", null);
    COOKIE="";
}
}
